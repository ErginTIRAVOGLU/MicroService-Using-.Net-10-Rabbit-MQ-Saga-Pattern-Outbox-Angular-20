    ...
    public static Action<HostBuilderContext, LoggerConfiguration> ConfigureLogger =>
        (context, loggerConfiguration) =>
        {
            ...
            if (context.HostingEnvironment.IsDevelopment())
            {
                ...
                loggerConfiguration.MinimumLevel.Override("OrderService", LogEventLevel.Debug);
            ...
            if (!string.IsNullOrEmpty(elasticUrl))
            {
                loggerConfiguration.WriteTo.Elasticsearch(new ElasticsearchSinkOptions(new Uri(elasticUrl))
                {
                    ...
                    IndexFormat = $"{env.ApplicationName.ToLower()}-{env.EnvironmentName.ToLower()}-{DateTime.UtcNow:yyyy-MM.dd}",
